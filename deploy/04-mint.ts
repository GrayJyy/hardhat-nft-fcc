...
const mint: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  log(`Basic NFT index 0 tokenURI: ${await basicNft.tokenURI(0)}`)
  ...
  const lowSvgImageUri = await dynamicSvgNft.lowSvg()
  const highSvgImageUri = await dynamicSvgNft.highSvg()
  log('----------------------------------------')
  log(lowSvgImageUri)
  log(highSvgImageUri)
  log('----------------------------------------')
