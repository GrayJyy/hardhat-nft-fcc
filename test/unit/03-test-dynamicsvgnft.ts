import { deployments, ethers, network } from 'hardhat'
import { developmentChains } from '../../helper-hardhat-config'
import { DynamicSvgNft, MockV3Aggregator } from '../../typechain-types'
import { assert, expect } from 'chai'

const LOW_SVG_IMAGE_URI =
  'data:image/svg+xml;base64,'
const HIGH_SVG_IMAGE_URI =
  'data:image/svg+xml;base64,'
const HIGH_TOKEN_URI =
  'data:application/json;base64,'
const LOW_TOKEN_URI =
  'data:application/json;base64,'

!developmentChains.includes(network.name)
  ? describe.skip
  : describe('DynamicSvgNft', () => {
      let dynamicSvgNft: DynamicSvgNft
      let mockV3Aggregator: MockV3Aggregator
      beforeEach(async () => {
        const { DynamicSvgNft, MockV3Aggregator } = await deployments.fixture(['dynamicsvgnft', 'mocks'])
        const dynamicSvgNftAddress = DynamicSvgNft.address
        const mockV3AggregatorAddress = MockV3Aggregator.address
        dynamicSvgNft = await ethers.getContractAt('DynamicSvgNft', dynamicSvgNftAddress)
        mockV3Aggregator = await ethers.getContractAt('MockV3Aggregator', mockV3AggregatorAddress)
      })
      describe('constructor', () => {
        it('Should gets the correct uri and initials the token id', async () => {
          const tokenId = await dynamicSvgNft.tokenId()
          assert.equal(tokenId.toString(), '0')
          const lowSvg = await dynamicSvgNft.lowSvg()
          assert.equal(lowSvg, LOW_SVG_IMAGE_URI)
          const highSvg = await dynamicSvgNft.highSvg()
          assert.equal(highSvg, HIGH_SVG_IMAGE_URI)
        })
      })
      describe('mintNft', () => {
        it('Should emits an event and creates the NFT', async () => {
          const value = ethers.utils.parseEther('1')
          expect(await dynamicSvgNft.mintNft(value)).to.emit(dynamicSvgNft, 'CreateNft')
          const valueToTokenId = await dynamicSvgNft.valueToTokenId(0)
          assert(valueToTokenId.toString(), '1')
          const tokenId = await dynamicSvgNft.tokenId()
          assert(tokenId.toString(), '1')
        })
        it("Should shifts the token uri to lower when the price doesn't surpass the highvalue", async () => {
          const value = ethers.utils.parseEther('1000000')
          const tx = await dynamicSvgNft.mintNft(value)
          await tx.wait()
          const svg = await dynamicSvgNft.tokenURI(0)
          assert(svg, LOW_TOKEN_URI)
          const value2 = ethers.utils.parseEther('1')
          const tx2 = await dynamicSvgNft.mintNft(value2)
          await tx2.wait()
          const svg2 = await dynamicSvgNft.tokenURI(1)
          assert(svg2, HIGH_TOKEN_URI)
        })
      })
    })
